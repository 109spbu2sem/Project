...
#ifndef Project_2Sem_IsPointOnSection_h
#define Project_2Sem_IsPointOnSection_h
...
class IsPointOnSection: public IConstraint
{
private:
    Point   *P;
    Section *S;
...
public:
    IsPointOnSection(Point & P1, Section & S1)
    {
        P = P1;
        S = S1;
    };
    ...
    {
        double Error = Length(S.A.X, S.A.Y, P.X, P.Y) + Length(S.B.X, S.B.Y, P.X, P.Y) - Length(S.A.X, S.A.Y, S.B.X, S.B.Y);  //  Length between two Points A and B of Section S == summ of Lengths between Point P and Points A and B of Section S => Point P is on Section S
