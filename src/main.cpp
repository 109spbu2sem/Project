...
int main()
{
	...
	storage_template<Point> s1; // помогает сократить объем кода
	Point p;
	s1.add(p);
	s1[0].x = 90;

	storage_template<Segment> ss1;
	// реализовать для списка destructor 
	/*
	while (true)
	{
		linkedlist l;
	...
	*/
	...
	cout << "Проверим на работаспосоность." << endl;

	//storagepoints storagep;
	/*storagep.add(p); // заком-ил
	storagep[1];
	Point p1 = storagep[1];
	...
	*/ 
	...
	for (unsigned i = 0; i < n; i++)
	{
		...
		//storage_s.add(Segment( p , p1 )); заком-ил
	}
